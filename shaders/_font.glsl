// Based on Renard's font.glsl
// https://github.com/Forenard/VJ-at-lambda-2023-06-10/blob/main/Shader/font.glsl
...
void SetFontStyle(int style) { Font_Style = style; }
...
void OffSetAnchor(vec2 offset) { Font_Anchor += offset * Char_Size; }

float _median(float r, float g, float b) { return max(min(r, g), min(max(r, g), b)); }
float Print_Char(vec2 uv, int id) {
    ...
    if (Font_Style == STYLE_BOLD || Font_Style == STYLE_OUTLINE ||
        Font_Style == STYLE_OUTLINE_ITALIC) {
        dist -= 0.3;
...
float Print_Number(vec2 uv, int value, int digit = -1) {
    float c = 0.0;
    value = abs(value);
    if (value == 0) return Print_Char(uv, C_0);
